...
- [x] 1.13 ~~**Cannot be automatically verified — manual device check only**~~: no white cold-start flash, system-bar icons legible with device set to light mode. **Reclassified and closed by automation (2026-09-02, after archive).** The original wording was CORRECT WHEN WRITTEN: design decision 9's reasoning — the Compose test harness launches its own `ComponentActivity`, not `MainActivity` with `Theme.Constanza` — held for a `createComposeRule` harness. What changed is that an emulator became part of the routine harness, and `ActivityScenario.launch(MainActivity::class.java)` launches the REAL Activity with the real theme, which the Compose rule never did. Now proved by `app/src/androidTest/kotlin/com/jjrapps/constanza/core/ui/DarkChromeInstrumentedTest.kt` (2 tests): (a) `windowBackgroundIsTheComposeBackground` resolves `android:windowBackground` off `R.style.Theme_Constanza` at runtime and asserts it equals `ConstanzaColors.Background.toArgb()` exactly, so the whole `themes.xml` → `colors.xml` → `ConstanzaColors` chain is under assertion, and it fails loudly if the attribute ever becomes a drawable rather than comparing a meaningless int; (b) `systemBarIconsStayLightWhileTheDeviceIsInLightMode` forces the device into light mode IN-PROCESS via `UiAutomation.executeShellCommand("cmd uimode night no")`, asserts `MainActivity`'s OWN configuration reports `UI_MODE_NIGHT_NO` (without that precondition a light-icon reading proves nothing — it is also the default), then asserts `isAppearanceLightStatusBars` and `isAppearanceLightNavigationBars` are both `false` via `WindowInsetsControllerCompat`. The device's original night-mode setting is captured and written back in `@After`, awaited not fired-and-forgotten, so no out-of-band `adb` setup is needed and the suite leaves the device as it found it. **Honest scope limit, stated in the test's own KDoc so a future reader cannot mistake it:** the transient white flash ITSELF is still not asserted — it is a frame-timing artifact needing a frame-capture harness this project does not have. What is asserted is the MECHANISM that prevents it, which design decision 9 already identified as the provable part: if the pre-Compose window colour equals the first Compose frame's colour, there is no colour for a flash to be.
...
- [ ] 6.5 ~~**Cannot be automatically verified — manual device matrix (final gate)**~~: notification accent shows the migrated colour on one posted reminder (`NotificationPoster.setColor`); dot renders correctly at `sw≥600dp` and across a fold/unfold configuration change. **Partially closed (2026-09-02, after archive) — LEFT OPEN deliberately, one part genuinely remains.** As with 1.13, the original "cannot be automatically verified" wording was correct when written and stopped being correct once an emulator joined the routine harness; it is reclassified, not retroactively judged.
  - [x] **Notification accent — CLOSED BY AUTOMATION.** `postedNotificationCarriesTheHabitColourAsItsAccent` in `app/src/androidTest/kotlin/com/jjrapps/constanza/reminding/NotificationPosterInstrumentedTest.kt` posts a REAL notification through `NotificationPoster.postReminder(...)` on the connected device and asserts `Notification.color` equals the `colorArgb` handed in, byte-for-byte. The input is `HabitColor.TEAL.argb` rather than a hex literal, so the test also documents the live palette instead of asserting against a colour the app can no longer produce. That is one half of the accent claim — `NotificationPoster.setColor` carries a habit's stored colour through to the system notification. The other half — that colours persisted before the palette change were actually rewritten on-palette — is proven by `AppDatabaseMigrationTest`, which asserts post-migration row VALUES rather than merely that `MIGRATION_1_2` completed. The two together close the claim end to end.
  - [x] **Colour dot at `sw >= 600dp` — already discharged during this change's own verification.** Evidence is `verify-report.md` ("Dot at sw≥600dp (6.5) — PROVEN"): `wm density 260` against a 1080px width yielded an effective ≈664dp, two real habits (pink, teal) rendered as visually distinct dots with row heights unchanged and no clipping, and the density was reset to the physical default (420) afterward. Recorded here for anyone re-running it: `Tablet7` (an existing AVD, natively 600dp wide) is the cleaner route — it needs no density override, and no physical device.
  - [ ] **Fold/unfold configuration change — STILL OPEN. Needs a foldable AVD or real foldable hardware; do not close this from a phone.** No device currently reachable from this machine can produce the state: the booted phone AVD reports only `DEFAULT` (`adb shell cmd device_state print-states`), and the attached physical device is a Galaxy S25 (`SM-S931B`, API 36) which reports only `DEFAULT` too — it is not the Galaxy Z Fold 7 (`SM-F966B`) this task's matrix named. **A foldable AVD IS creatable from the installed SDK**, confirmed rather than assumed: the `pixel_fold`, `pixel_9_pro_fold` and `pixel_10_pro_fold` skins are installed, and `sdklib.jar` carries the `pixel_fold` device definition with a `<d:foldable-region>` and `change-orientation-on-fold`, against the installed `android-37.0` image. The catch is the route: the `cmdline-tools` SDK package is NOT installed, so there is no `avdmanager` binary — creation has to go through Android Studio's Device Manager, or `cmdline-tools` has to be installed first. **Note for whoever routes this:** `habit-tracking-mvp`'s own G.7 matrix already ran a fold/unfold on the Fold 7 and passed it (see `carried_forward_open_items.G.7-throttling-row.reason` in `openspec/config.yaml`), but that assertion was about the habit EDITOR surviving the configuration change. This one is about the habit colour DOT still rendering across it — a different assertion on a different screen, so that PASS does not transfer.
  - If `IllegalStateException: No compose hierarchies found` appears on any device, check `adb shell dumpsys window | rg isKeyguardShowing` and wakefulness FIRST — it means no Activity resumed, not a code fault (`app/build.gradle.kts:178-186`). This has been misdiagnosed three times on this project.
...
| Cold-start dark window, pinned system-bar icons | spec `Cold-Start Window Background And System Bar Icons` | 1.7, 1.8, 1.9, 1.13 (**now automated** — `DarkChromeInstrumentedTest`; the transient flash itself remains unasserted, the mechanism does not) |
...
| Manual — physical device | Pixel 10 (API 37), Galaxy Z Fold 7 (SM-F966B, API 36) | ~~1.13 (cold-start flash — no automated harness exists), 6.5 (notification accent, fold/unfold)~~ **Superseded 2026-09-02**: 1.13 and 6.5's notification accent are now instrumented (`DarkChromeInstrumentedTest`, `NotificationPosterInstrumentedTest`) and need no physical device. Only 6.5's **fold/unfold** still requires a foldable — AVD or hardware. |
