...
- [x] 1.1 `app/src/androidTest/kotlin/com/jjrapps/constanza/habit/HabitRepositoryTestFixture.kt`: hoist
...
- [x] 1.2 `app/src/androidTest/kotlin/com/jjrapps/constanza/e2e/CoreFlowTestFixture.kt`: add a bounded
...
- [x] 2.1 `app/src/main/kotlin/com/jjrapps/constanza/core/data/dao/Daos.kt`: add
...
- [x] 2.2 `HabitDaos`: add `reminderOccurrenceDao` field (5th field).
- [x] 2.3 `app/src/main/kotlin/com/jjrapps/constanza/habit/HabitRepository.kt`: constructor gains
...
- [x] 3.1 `HabitListViewModel.kt`: add `delete(habitId: Long)` calling `repository.delete`.
- [x] 3.2 `HabitListScreen.kt`: add trailing `IconButton(Icons.Filled.MoreVert)` with a `DropdownMenu`
      (single item: Delete) to `HabitRow.trailingContent`, beside the existing Progress/Archive
      `TextButton`s.
- [x] 3.3 `HabitListScreen.kt`: `DeleteHabitDialog` (M3 `AlertDialog`, matching
...
- [x] 3.4 `res/values/strings.xml`: menu label, dialog title, `<plurals>` body (`one`/`other`),
      confirm label; dismiss reuses `action_cancel`.

## Phase 4: Tests

- [x] 4.1 Repository instrumented test: delete a habit with a schedule, reminder slots, and entries;
...
- [x] 4.2 Repository instrumented test, using the hoisted mock from 1.1:
      ...
      **first** line of defence — cancellation actually ran — not that a later broadcast is ignored
      (that's 4.9).
- [x] 4.3 Repository instrumented test: two habits, one archived; delete the active one; assert the
      archived habit's `archived`, `archivedAt`, and entries are unchanged. Proves: deletion does not
      affect archiving.
- [x] 4.4 Repository instrumented test: delete a zero-entry habit; same outcome as 4.1 minus entries.
      Proves: no-history parity.
- [x] 4.5 Compose test (`HabitRepositoryTestFixture.register`): overflow opens; dialog shows habit
      name and "7 recorded answers"; a zero-history habit shows "0 recorded answers". Proves: D3/D4
      count rendering.
- [x] 4.6 Compose test: declining the dialog leaves every table unchanged.
- [x] 4.7 Manual/visual check on the API 31 emulator: confirm two `TextButton`s + the new 48dp
...
- [x] 4.8 `CoreFlowE2ETest.kt`: rewrite the `:309-317` KDoc rationale for
...
- [x] 5.1 `./gradlew check` (no instrumented coverage; confirms compile + unit/lint gates).
- [x] 5.2 `./gradlew :app:emulatorMatrixGroupDebugAndroidTest`, API 31 + API 37, nothing attached.
