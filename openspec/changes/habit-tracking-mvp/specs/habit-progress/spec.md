...
The `:domain` module MUST expose a pure function computing the current streak from `Entry` history, with no I/O. `SKIPPED` entries MUST NOT break a streak and MUST NOT themselves count as a completed day — a `SKIPPED` day bridges the run without lengthening it. Only `COMPLETED` days lengthen a streak; only `MISSED` breaks one. This holds wherever the day sits: an enclosed pass-through day and a trailing one MUST be treated identically, so that marking a two-week holiday as skipped preserves a streak without crediting fourteen days the user did not complete. `UNKNOWN` entries MUST likewise NOT break a streak and MUST NOT themselves count as a completed day — an occurrence still pending behind a live snooze passes through unaffected, exactly like `SKIPPED`. Only `MISSED` breaks a streak. An `Entry` later corrected to `COMPLETED` (from either `MISSED` or a pending `UNKNOWN`) MUST be reflected by any streak computation run after the correction.

#### Scenario: A skipped day bridges a streak without lengthening it
...
- THEN it reports an unbroken 8-day streak — the run is not broken, and the skipped day itself
  contributes nothing, because only completed days lengthen a streak

#### Scenario: An enclosed unknown day behaves exactly like an enclosed skipped day
- GIVEN a streak of 5 completed days followed by 1 `UNKNOWN` day, followed by 3 more completed days
- WHEN the streak function evaluates today
- THEN it reports an unbroken 8-day streak, identical to the skipped case
...
The `:domain` module MUST expose a pure function computing compliance as `completed / (completed + missed)` over a caller-supplied window length in days, except for `N_TIMES_PER_WEEK`, whose unit of obligation is the week (design D8): for that kind the ratio MUST be the sum of `min(completedInWeek, n)` over the whole weeks in the window, divided by the sum of `n` over those same weeks, with partial weeks at the window edge excluded — the window MUST NOT be hard-coded inside the function. `SKIPPED` and `UNKNOWN` entries MUST be excluded from both the numerator and the denominator.
...
#### Scenario: An N-times-per-week habit is measured in weeks, not days
- GIVEN a habit of 3 times per week, and a window covering 2 whole weeks
- AND the first week has all 3 completions, on Monday, Wednesday and Friday
- AND the second week has only 2 completions
- WHEN compliance is computed
- THEN the ratio is `(3 + 2) / (3 + 3)`, and the four non-completion days of the first week are NOT
  counted as failures, because the week's quota was met regardless of which days were used
