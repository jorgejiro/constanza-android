...
- [ ] 2.1 RED: add `HabitListScreen` compose tests — overflow launcher only rendered, no always-visible buttons; opening it shows Progress, Archive/Un-archive, Delete; a name fitting two lines shows no ellipsis; a longer name ellipsizes at two lines.
- [ ] 2.2 `habit/HabitListScreen.kt`: `HabitRow.trailingContent` → `Box { IconButton + DropdownMenu }` (Progress, Archive/Un-archive, Delete, each resetting `menuExpanded = false`); delete `supportingContent`; `headlineContent` gets `maxLines = 2, overflow = Ellipsis`; rewrite the KDoc to record the D2 supersession.
- [ ] 2.3 Repoint menu-by-text sites to open the overflow first (reuse `HabitDeleteDialogComposeTest.kt:64`'s pattern): `e2e/CoreFlowE2ETest.kt:336`, `habit/HabitListArchiveComposeTest.kt:89,95,101` (reopen before the final `assertExists()`).
- [ ] 2.4 `habit/HabitEditorScreen.kt`: notes `OutlinedTextField` gains `minLines = 3, maxLines = 5`.
- [ ] 2.5 `core/data/migration/PreMigrationSnapshotWriter.kt`: derive the filename from `db.version` (`pre-migration-v${db.version}.sql`/`.tmp`), replacing the hardcoded `pre-migration-v1.sql` + `REPLACE_EXISTING` (design D3).
- [ ] 2.6 GREEN: run 2.1 plus the full instrumented matrix (Unit 2); confirm `AppDatabaseMigrationTest`'s existing 1→2 snapshot-path assertion stays byte-identical green.
